...
function collectSections(suite: any): Section[] {
  const sections: Section[] = [];
  if (suite.specs && suite.specs.length > 0) {
    const tests: TestEntry[] = suite.specs.map((spec: any) => {
...
function specToTitle(file: string): string {
...
function folderTitle(files: string[]): { title: string; slug: string } {
  // Find common parent folder among all spec files
  const dirs = files.map(f => path.dirname(f).split('/').pop() ?? '');
  const unique = [...new Set(dirs.filter(Boolean))];
  if (unique.length === 1 && unique[0]) {
    const slug = unique[0].replace(/[^a-zA-Z0-9]/g, '-');
    const title = unique[0].replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    return { title, slug };
  }
  return { title: 'Test Report', slug: 'test-report' };
}

function buildCombinedHtml(reportTitle: string, stats: any, fileBlocks: FileBlock[]): string {
...
<div class="page-header">
  ...
  <div class="subtitle">Combined report &mdash; ${fileBlocks.length} spec file${fileBlocks.length !== 1 ? 's' : ''}</div>
</div>
...
export default async function globalTeardown() {
  ...
  if (!fs.existsSync(jsonPath)) {
    console.log('\nNo results.json found — skipping PDF generation.');
    return;
  }
  ...
  try {
    ...
    console.log('\nCould not parse results.json — skipping PDF generation.');
    return;
  }

  const topSuites: any[] = report.suites ?? [];
  if (topSuites.length === 0) return;
  ...
  for (const topSuite of topSuites) {
    const file: string = topSuite.file || topSuite.title || 'unknown';
    ...
    files.push(file);
    fileBlocks.push({ title: specToTitle(file), sections });
  }
  ...
  const { title: reportTitle, slug } = folderTitle(files);
  ...
  try {
    ...
    await page.close();
    console.log(`\n✓ Combined PDF report saved: ${pdfPath}`);
  } finally {
