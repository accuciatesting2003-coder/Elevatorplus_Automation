...
async function gotoBranchMaster(page: any) {
  ...
  if (page.url().includes('/login')) {
    ...
    if (!page.url().includes('branch-master')) {
      await page.goto(BRANCH_MASTER_URL);
      await page.waitForLoadState('networkidle').catch(() => {});
    }
  }

  await page.getByRole('heading', { name: 'Add Branch' }).waitFor({ state: 'visible', timeout: 60000 });
  await dismissNotificationPopup(page);
}
...
test.describe('Add Branch', () => {
  ...
  test.only('1.1 Add branch with all valid fields (with location from map)', async ({ page }) => {
  ...
  test('1.4 Newly added branch appears in the table', async ({ page }) => {
    ...
    const branchCode = `VB${timestamp.toString().slice(-4)}`;

    await page.getByRole('textbox', { name: 'Branch Name *' }).fill(branchName);
    await page.getByRole('textbox', { name: 'Branch Code *' }).fill(branchCode);
    await getBranchAddressField(page).fill('Verify Address');
    ...
    await expect(page.locator('table tbody').getByText(branchName)).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Branch Master UI', () => {
  ...
  test('3.5 Branch table is visible with correct headers', async ({ page }) => {
    ...
    await expect(page.locator('table').getByText('Actions')).toBeVisible();
    await expect(page.locator('table').getByText('Branch Name')).toBeVisible();
    await expect(page.locator('table').getByText('Branch Code')).toBeVisible();
    await expect(page.locator('table').getByText('Branch Address')).toBeVisible();
  ...
  test('3.7 Search box is visible and functional', async ({ page }) => {
    ...
    await expect(page.locator('table tbody').getByText('Pune')).toBeVisible();
  });
  ...
  test('3.11 Clicking Edit loads branch data into the form', async ({ page }) => {
    ...
    const firstRowBranchName = await page.locator('table tbody tr').first().locator('td').nth(2).textContent();
