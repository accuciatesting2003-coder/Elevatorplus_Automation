...
The ARD Form page is part of the ElevatorPlus Sales Forms section. It allows administrators to manage ARD configuration records used in elevator sales. The page includes a form to add/update ARD records and a searchable, filterable data table with status management (Active/Inactive).
...
**Uniqueness rule:** The combination of Type of Lift + Type of Machine + Lift Type + Passenger must be unique across all records (both Active and Inactive).
...
#### 1.1. TC-ARDF-001: Verify ARD Form page loads successfully

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Log in to the admin panel using valid credentials (Mobile: 9209365301, Password: Shravani@123).
  2. Navigate to the ARD Form from the left sidebar (Sales Forms > ARD).
    - expect: The ARD Form page loads correctly with the form visible.
    - expect: All five fields are visible: Type of Lift, Type of Machine, Lift Type, Passenger, and Price.
    - expect: The data table is displayed below the form.
    - expect: The Clear and Submit buttons are both visible.

#### 1.2. TC-ARDF-002: Verify page title reads "machine form"

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page.
...
#### 1.3. TC-ARDF-003: Verify sub-title reads "add machine form details"
...
#### 2.1. TC-ARDF-004: Verify validation messages appear for all mandatory fields when form is submitted empty
...
**Steps:**
  ...
  3. Click the Submit button.
    - expect: Validation error messages appear for all 4 mandatory fields: Type of Machine, Lift Type, Passenger, and Price.
...
#### 2.2. TC-ARDF-005: Verify validation message for Type of Machine when left unselected
...
#### 2.3. TC-ARDF-006: Verify validation message for Lift Type when left unselected
...
#### 2.4. TC-ARDF-007: Verify validation message for Passenger when left unselected
...
#### 2.5. TC-ARDF-008: Verify validation message for Price when left empty
...
#### 3.1. TC-ARDF-009: Verify Price field rejects whitespace-only input

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page.
  2. Fill all mandatory dropdown fields with valid selections (Type of Machine, Lift Type, Passenger).
...
#### 4.1. TC-ARDF-010: Verify Price rejects alphabetic characters
...
#### 4.2. TC-ARDF-011: Verify Price rejects negative values
...
#### 4.3. TC-ARDF-012: Verify Price accepts zero (0)
...
#### 5.1. TC-ARDF-013: Verify Clear button resets the Add form
...
#### 5.2. TC-ARDF-014: Verify Clear button clears validation error messages
...
#### 5.3. TC-ARDF-015: Verify Clear button in Edit/Update mode resets form to Add mode

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click the Edit (pencil icon) button on an existing record in the data table.
...
#### 6.1. TC-ARDF-016: Add record with all fields using single values
...
**Steps:**
  ...
  7. Click the Submit button.
    ...
    - expect: The new ARD record appears in the data table with the correct values and Status 'Active'.

#### 6.2. TC-ARDF-017: Add record with multi-select values for Type of Lift and Passenger
...
#### 6.3. TC-ARDF-018: Add record with only mandatory fields (skip optional Type of Lift)
...
> **Uniqueness key:** The server enforces uniqueness on the four-field combination: **Type of Lift + Type of Machine + Lift Type + Passenger**. The duplicate check applies to both Active and Inactive records.

#### 7.1. TC-ARDF-019: Adding the exact same combination with multiple values shows duplicate error

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page.
  2. Select multiple values in Type of Lift, one Type of Machine, one Lift Type, and multiple Passengers. Enter a Price. Click Submit.
    - expect: First record is created successfully (success toast shown, record appears in table).
  3. In the form, re-select the exact same combination: same Type of Lift values, same Type of Machine, same Lift Type, same Passenger values. Enter any Price. Click Submit.
    - expect: The system displays a duplicate entry error message.
    - expect: No new record is created in the data table.

#### 7.2. TC-ARDF-020: Adding a single-value combination that matches an existing active record shows error

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Identify an existing active record in the data table. Note its exact combination of Type of Lift, Type of Machine, Lift Type, and Passenger values.
...
#### 7.3. TC-ARDF-021: Adding a unique multi-select combination creates a new record successfully

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page.
  2. Select a combination of Type of Lift, Type of Machine, Lift Type, and Passenger values that does NOT match any existing record (use different values or a different number of selected options). Enter a valid Price.
  3. Click the Submit button.
    - expect: A success toast notification is shown.
    - expect: The new record appears in the data table.
    - expect: No duplicate error is shown.

#### 7.4. TC-ARDF-022: Submit two combinations where one matches an existing active record — only the duplicate fails

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Identify:
  ...
  2. Submit Combination A (select its values, enter a Price, click Submit).
...
#### 7.5. TC-ARDF-023: Submit two combinations where one matches an existing inactive record — duplicate still blocked

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Use the Status filter to view Inactive records. Identify the combination (Type of Lift, Type of Machine, Lift Type, Passenger) of an existing inactive record. Note it as Combination X.
  2. Note a second completely unique combination as Combination Y that does not exist in either active or inactive records.
  3. Switch Status filter back to Active. Submit Combination Y (enter a Price, click Submit).
...
#### 8.1. TC-ARDF-024: Verify Edit functionality opens form with pre-populated values

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and locate an existing record in the data table.
  2. Click the Edit (pencil icon) button on the record.
    - expect: The form switches to Update mode.
    - expect: All five fields are pre-populated with the existing record's values.
...
#### 8.2. TC-ARDF-025: Successfully update an existing ARD record with valid new values

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click Edit on an existing record.
    - expect: Update form opens with current values pre-filled.
  2. Modify one or more fields (e.g., change the Price, or change one of the dropdown values) to new valid values that form a unique combination.
...
#### 8.3. TC-ARDF-026: Updating a record to match another existing active combination shows error

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Identify two distinct active records — Record A and Record B. Note the exact combination (Type of Lift, Type of Machine, Lift Type, Passenger) of Record A.
  2. Click Edit on Record B to open it in Update mode.
    - expect: Update form opens with Record B's current values.
  3. Change Record B's dropdown selections to exactly match Record A's combination (Type of Lift, Type of Machine, Lift Type, Passenger). Click the Update button.
    - expect: The system displays a duplicate entry error message.
...
#### 8.4. TC-ARDF-027: Updating a record to match an existing inactive combination shows error

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Change the Status filter to 'Inactive'. Note the exact combination of an existing inactive record (Combination Z).
...
#### 8.5. TC-ARDF-028: Updating a record with its own same combination does not show duplicate error

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click Edit on an existing active record.
    - expect: Update form opens with the record's current values pre-filled.
  2. Without changing any of the uniqueness-determining dropdown fields (Type of Lift, Type of Machine, Lift Type, Passenger), only modify Price.
  3. Click the Update button.
...
#### 8.6. TC-ARDF-029: Verify Clear button in Update mode discards changes and reverts to Add mode

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click Edit on an existing record to open Update mode.
...
#### 8.7. TC-ARDF-030: Verify mandatory field validation on Update form when all mandatory fields are cleared
...
**Steps:**
  ...
  2. Deselect all values from every mandatory dropdown field (Type of Machine, Lift Type, Passenger) and clear the Price field. Leave Type of Lift as-is (optional).
  3. Click the Update button.
    - expect: Validation error messages appear for all 4 mandatory fields: Type of Machine, Lift Type, Passenger, and Price.
    - expect: No validation error appears for Type of Lift (it is optional).
    - expect: The record is not updated. The form remains in Update mode.

#### 8.8. TC-ARDF-031: Verify validation error for Type of Machine when unselected during Update
...
#### 8.9. TC-ARDF-032: Verify validation error for Lift Type when unselected during Update
...
#### 8.10. TC-ARDF-033: Verify validation error for Passenger when unselected during Update
...
#### 8.11. TC-ARDF-034: Verify validation error for Price when cleared during Update
...
#### 8.12. TC-ARDF-035: Two unique update combinations — one matching existing active, one matching existing inactive — both blocked

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Identify one existing active record and one existing inactive record with known combinations. Note them as Combo-Active and Combo-Inactive.
...
#### 9.1. TC-ARDF-036: Mark an Active record as Inactive via Edit

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page with the default Active filter applied. Note the combination details of the first record (Record X).
...
#### 9.2. TC-ARDF-037: Re-activate an Inactive record via Edit

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Change the Status filter to 'Inactive'. Click Edit on an Inactive record.
...
#### 9.3. TC-ARDF-038: Verify default Active filter is applied on page load
...
**Steps:**
  ...
  2. Observe the Status filter dropdown in the data table toolbar without making any changes.
    ...
    - expect: Only active ARD records are displayed in the data table.

#### 9.4. TC-ARDF-039: Verify Active, Inactive, and All status filter options work correctly

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Ensure both active and inactive records exist.
...
#### 10.1. TC-ARDF-040: Verify search filters the data table in real time
...
#### 10.2. TC-ARDF-041: Verify search with no matching results shows empty state

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page.
  2. Type a search term that does not match any ARD record (e.g., 'XYZNOTEXIST99999') in the search field.
...
#### 11.1. TC-ARDF-042: Verify rows-per-page dropdown changes the number of displayed rows

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Locate the 'Show:' dropdown in the data table toolbar. Note the default value is '25'.
...
#### 11.2. TC-ARDF-043: Verify pagination controls navigate between pages correctly

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page. Set the Show dropdown to '10'. Verify there are multiple pages of records.
...
#### 12.1. TC-ARDF-044: Verify Update Price button opens the bulk price update modal
...
**Steps:**
  ...
  2. Click the 'Update Price' button in the data table toolbar.
    ...
    - expect: The modal lists ARD records with columns showing current configuration details and an editable 'New Price' input field per record.
    - expect: A search input is visible within the modal.
    - expect: Submit/Update and Cancel buttons are visible in the modal.

#### 12.2. TC-ARDF-045: Verify bulk price update with valid new price values updates the data table

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click 'Update Price'.
...
#### 12.3. TC-ARDF-046: Verify Cancel button closes the Update Price modal without saving changes
...
#### 12.4. TC-ARDF-047: Verify close (X) button closes the Update Price modal without saving changes
...
#### 12.5. TC-ARDF-048: Verify search within the Update Price modal filters records

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Navigate to the ARD Form page and click 'Update Price'.
    - expect: Bulk price update modal opens with the full list of ARD records.
...
#### 12.6. TC-ARDF-049: Verify clearing the search input in Update Price modal restores full record list
...
**Steps:**
  ...
  3. Clear the search input completely.
    - expect: The full list of ARD records is restored in the modal.

#### 12.7. TC-ARDF-050: Verify Update Price modal new price field rejects negative values
...
#### 12.8. TC-ARDF-051: Verify Update Price modal new price field rejects alphabetic characters
...
#### 12.9. TC-ARDF-052: Verify Update Price modal new price field accepts zero (0)
...
#### 13.1. TC-ARDF-053: Unauthenticated access to ARD Form URL redirects to login

**File:** `tests/sales-forms/ard-form.spec.ts`

**Steps:**
  1. Open a new browser context (no authentication state) and navigate directly to the ARD Form URL.
    - expect: The user is redirected to the login page at https://stage.elevatorplus.net/login.
    - expect: The ARD Form page content (form and table) is not shown.

#### 13.2. TC-ARDF-054: Access ARD Form via sidebar navigation
...
**Steps:**
  ...
  2. Click the 'ARD' link in the sub-menu.
    - expect: The ARD Form page loads correctly.
